...
$items = !is_array($logicData)?Convert::By($logicData):logic(grab($logicData, "LogicName") ?? "content/all", [
    "Filter" => [
...
view(grab($viewData, "ViewName") ?? "contents", [
    "Title" => grab($viewData, "Title") ??  between($type, $tag, $cat, preg_replace("/\..*$/", "", \Req::$Page), grab($data, "DefaultTitle") ?? "Contents"),
