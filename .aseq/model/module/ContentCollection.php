...
class ContentCollection extends Collection{
    ...
    */
	...
	function __construct($items = null){
        ...
        $this->CollectionRoot = $this->CollectionRoot??\_::$Router->CategoryRoot;
				    ...
				    if($this->AllowAuthor)
                        doValid(
                            function($val) use(&$p_meta){
                                ...
                                if(!isEmpty($authorName)) $p_meta .=  " ".Struct::Link($authorName["Name" ],\_::$Router->UserRoot.$authorName["Signature" ],["class"=>"author"]);
                            },
                            $item,
                            'AuthorId'
                        );
