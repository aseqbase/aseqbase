...
class MainMenu extends Module{
	...
	public function Echo(){
		?>
			...
			if($count > 0){
				...
				for($i = 0; $i < $count; $i++){
					 echo $this->CreateItem($this->Items[$i],1); 
				} 
				echo "</ul>";
			}
	...
	protected function CreateItem($item, $ind = 1){
		$act = (endsWith($_SERVER['REQUEST_URI'],$item['Link'])?'active':'');
		$ret = "";
		$ind++;
		if(isValid($item, "Items")) {
			$ret .= "<li class='DropDown $act'>
				<a ".(isValid($item,"Attributes")?$item['Attributes']:"")." href='".(isValid($item,"Name")?$item['Link']:"")."'>
					<div class='box'>".(isValid($item,"Name")?__($item['Name']):"")."</div>
				</a>";
			$count = count($item["Items"]);
			if($count > 0){
				$ret .= "<ul class='Sub-Items Sub-Items-$ind'>";
				for($i = 0; $i < $count; $i++){
					$ret .= $this->CreateItem($item["Items"][$i], $ind);
				}
				$ret .= "</ul>";
			}
			$ret .= "</li>";
