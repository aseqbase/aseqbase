				...
				<?php for($i = 0; $i < $count; $i++){ ?>
							<div class="col-md">
								<div class="teammember" data-aos="down">
									<div class="image">
										<img src="<?php echo $menu[$i]['Image']; ?>">
									</div>
									<div class="title">
										<sup><?php echo __($menu[$i]['PreName']); ?></sup>
										<div><strong><?php echo __($menu[$i]['FirstName'])." ".__($menu[$i]['MiddleName'])." ".__($menu[$i]['LastName']); ?></strong></div>
										<sub><?php echo __($menu[$i]['PostName']); ?></sub>
									</div>
									<div class="features">
									<?php foreach($menu[$i]['Assignees'] as $assignee) { ?>
										<div> <?php echo __($assignee,true,false); ?> </div><br>
									<?php } ?>
									</div>
									<ul class="list-group">
										<?php foreach($menu[$i]['Items'] as $item) { ?>
											<li class="list-item d-flex justify-content-between align-items-center">
												<i class="fa <?php echo $item['Class'] ?>" aria-hidden="true">
												<?php echo __($item['Key'],true,false).__(":",true,false); ?>
												</i>
												<?php echo __($item['Value'],true,false); ?>
											</li>
										<?php } ?>
									</ul>
								<a class="btn" target="blank" href="<?php echo $menu[$i]['Link']; ?>"><?php echo __($this->MoreButtonLabel); ?></a>
								</div>
							</div>
				<?php } ?>
			</div>
