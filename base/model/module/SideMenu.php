			...
			?>
				<div class="container" onclick="<?php echo $this->Name; ?>_ViewSideMenu(false)">
					...
					for($i = 0; $i < $count; $i++){
						...
						$ll = $sl;
						?>
							<li class="item col-sm <?php echo (endsWith(\_::$URL,$this->Items[$i]['Link'])?'active':''); ?>">
								<a <?php echo $this->Items[$i]['Attributes'] ?> href="<?php echo $this->Items[$i]['Link'] ?>">
								<div class="box"><?php echo __($this->Items[$i]['Name']) ?></div>
								</a>
							</li>
					<?php } ?>
						</div>
