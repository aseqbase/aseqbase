...
if (!isValid($path) && (\_::$Address->UrlRoute === "home" || isEmpty(\_::$Address->UrlRoute)))
    ...
    $doc = compute(get($computeData, "ComputeName")??"content/get", [ "Name" => get($computeData, "Name")??urldecode(\_::$Address->UrlRoute),...($computeData??[]) ]);
    if(isEmpty($doc)) page(between($path, urldecode(\_::$Address->UrlRoute)));
