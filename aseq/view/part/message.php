...
module("Field");
$form->Items = [new MiMFa\Module\Field("text", "Subject", $msg, required: true)];
if (!\_::$User->Email) {
	$form->Items[] = new MiMFa\Module\Field("text", "Name", $name, required: true, lock: !isEmpty($name));
	$form->Items[] = new MiMFa\Module\Field("email", "Email", $email, required: true, lock: !isEmpty($email));
}
$form->Items[] = new MiMFa\Module\Field("texts", "Message", $msg, required: true);
pod($form, $data);
$form->Render();