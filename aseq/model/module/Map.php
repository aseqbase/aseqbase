...
class Map extends Module
{
	...
	{
		if ($this->Local) {
			$this->StyleSource = asset(\_::$Address->StructRootDirectory, "Map/Map.css");
			$this->ScriptSource = asset(\_::$Address->StructRootDirectory, "Map/Map.js");
		}
		$this->Location = $this->Location ?? get($this->Item = $this->Item ?? first($this->Items), "Location") ?? [0, 0];
		if ($this->Local) {
			$this->PluginsStyleSources[] = asset(\_::$Address->StructRootDirectory, "Map/Button.css");
			$this->PluginsScriptSources[] = asset(\_::$Address->StructRootDirectory, "Map/Button.js");
		} else {
		...
		if ($this->DrawPosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->StructRootDirectory, "Map/Draw.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->StructRootDirectory, "Map/Draw.js");
			} else {
		...
		if ($this->LocatePosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->StructRootDirectory, "Map/Locate.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->StructRootDirectory, "Map/Locate.js");
			} else {
		...
		if ($this->FullScreenPosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->StructRootDirectory, "Map/FullScreen.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->StructRootDirectory, "Map/FullScreen.js");
			} else {
		...
		if ($this->PrintPosition) {
			if ($this->Local)
				$this->PluginsScriptSources[] = asset(\_::$Address->StructRootDirectory, "Map/Print.js");
			else
				$this->PluginsScriptSources[] = "https://unpkg.com/leaflet.browser.print/dist/leaflet.browser.print.min.js";
		}
		if ($this->CompassPosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->StructRootDirectory, "Map/Compass.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->StructRootDirectory, "Map/Compass.js");
			} else {
		...
		if ($this->MeasurePosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->StructRootDirectory, "Map/Measure.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->StructRootDirectory, "Map/Measure.js");
			} else {
