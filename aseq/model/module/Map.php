...
class Map extends Module
{
	...
	{
		if ($this->Local) {
			$this->StyleSource = asset(\_::$Address->GlobalStructDirectory, "Map/Map.css");
			$this->ScriptSource = asset(\_::$Address->GlobalStructDirectory, "Map/Map.js");
		}
		$this->Location = $this->Location ?? get($this->Item = $this->Item ?? first($this->Items), "Location") ?? [0, 0];
		if ($this->Local) {
			$this->PluginsStyleSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Button.css");
			$this->PluginsScriptSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Button.js");
		} else {
		...
		if ($this->DrawPosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Draw.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Draw.js");
			} else {
		...
		if ($this->LocatePosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Locate.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Locate.js");
			} else {
		...
		if ($this->FullScreenPosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/FullScreen.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/FullScreen.js");
			} else {
		...
		if ($this->PrintPosition) {
			if ($this->Local)
				$this->PluginsScriptSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Print.js");
			else
				$this->PluginsScriptSources[] = "https://unpkg.com/leaflet.browser.print/dist/leaflet.browser.print.min.js";
		}
		if ($this->CompassPosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Compass.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Compass.js");
			} else {
		...
		if ($this->MeasurePosition) {
			if ($this->Local) {
				$this->PluginsStyleSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Measure.css");
				$this->PluginsScriptSources[] = asset(\_::$Address->GlobalStructDirectory, "Map/Measure.js");
			} else {
