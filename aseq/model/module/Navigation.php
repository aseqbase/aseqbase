...
class Navigation extends Module
{
		...
		if (is_string($itemsOrQuery)) {
			...
			if ($this->LiveCount || $this->Count <= 0)
				if (isValid($count))
					$this->Count = $count;
				elseif ($this->AllowCount)
					$this->Count = count(Convert::ToItems(between($this->Items, $this->DefaultItems)));
				else
					$this->Count = 12;
		}
	...
	{
		if (isValid($iterator ?? $this->Items))
			return array_slice(Convert::ToItems($iterator ?? $this->Items), $this->GetFromItem(), $this->GetLimit());
		elseif ($this->Query)
			return between(\_::$Back->DataBase->FetchRowsExecute($this->Query . " LIMIT " . $this->GetFromItem() . ", " . $this->GetLimit(), $this->QueryParameters), array_slice($this->DefaultItems, $this->GetFromItem(), $this->GetLimit()));
	}
